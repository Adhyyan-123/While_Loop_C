...
%token <nam> ID NUM WHILE REL VOID INCLUDE RETURN INT FLOAT CHAR MAIN  PRINTF STRL
%type <nam> E V S 
%right '='
%left '+' '-'
%left '*' '/'
...
E :V '='{push();add('o');} E{codegen_assign();syntax[k].left = $1.name; syntax[k].right= "expr"; syntax[k].token = "=";k++;}
  ...
  | E '/'{push();add('o');} E{codegen();syntax[k].left = $1.name; syntax[k].right= $4.name; syntax[k].token = "/";k++;}
  | E REL{push();add('r');} E{codegen();syntax[k].left = $1.name; syntax[k].right= $4.name; syntax[k].token = strdup($2.name);k++;}
  | '('{add('t');} E ')'{add('t');}
